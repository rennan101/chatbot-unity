...
window.meuPeer = null;
window.streamLocalAudio = null;
window.streamLocalVideo = null;
window.chamadasAtivas = {};
window.peerConfigurado = false;
...
// ==========================================================
// 2. PRESENÇA EM TEMPO REAL BLINDADA (VARREDURA GLOBAL)
// ==========================================================
...
window.limparMinhaPresencaGlobal = async function() {
    ...
    window.projetos.forEach((proj) => {
        ...
        // Remove a presenca do avatar
        if (proj.presenca && proj.presenca[window.usuarioAtual.email] !== undefined) {
            updates[`presenca.${window.usuarioAtual.email}`] = deleteField();
            needsUpdate = true;
        }
        
        // Remove a tela e a voz
        if (proj.conversas) {
            let convUpdated = false;
            proj.conversas.forEach((c) => {
                if (c.tela === window.usuarioAtual.email) {
                    c.tela = null;
                    convUpdated = true;
                }
                if (c.chamada && c.chamada[window.usuarioAtual.email]) {
                    delete c.chamada[window.usuarioAtual.email];
                    convUpdated = true;
                }
            });
            if (convUpdated) {
    ...
    if (window.meuPeer) { window.sairDaChamada(true); }
...
window.iniciarEscutaUsuarios = function() {
    ...
    }, (error) => { console.warn("Escuta restrita."); });
}
...
window.confirmarLogout = async function() {
    if (confirm("Tem certeza que deseja sair da sua conta?")) {
        // Desligamos os hooks para evitar erros no console
        if (window.unsubscribeProjetos) window.unsubscribeProjetos();
        if (window.unsubscribeConvites) window.unsubscribeConvites();
        if (window.unsubscribeNotificacoes) window.unsubscribeNotificacoes();
        if (window.unsubscribeUsuarios) window.unsubscribeUsuarios();
...
window.renderizarTags = function() {
    ...
    TAGS_DISPONIVEIS.forEach(tag => {
        ...
        span.innerText = tag;
        span.onclick = () => {
            if(window.tagsSelecionadas.includes(tag)) { window.tagsSelecionadas = window.tagsSelecionadas.filter(t => t !== tag); } 
            else { window.tagsSelecionadas.push(tag); }
...
window.salvarPerfil = async function() {
    ...
    try {
        if(nome && nome !== window.usuarioAtual.displayName) {
            await updateProfile(window.usuarioAtual, { displayName: nome });
        }
        await setDoc(doc(db, "usuarios", window.usuarioAtual.uid), {
            nome: nome, profissao: profissao, tags: window.tagsSelecionadas, email: window.usuarioAtual.email
        }, { merge: true });
...
window.responderConvite = async function(conviteId, projetoId, remetente, projetoNome, aceitar) {
    ...
    try {
        if (aceitar && projetoId) {
            await updateDoc(doc(db, "projetos", projetoId), { membros: arrayUnion(window.usuarioAtual.email) });
            mostrarToast(getMeme('sucesso'), "rgba(46, 204, 113, 0.9)", SVG_CHECK);
        } else {
...
document.addEventListener('touchend', e => {
    ...
    // Ignora swipes se foi um scroll vertical ou em elementos como range e blocos de código
    ...
    if (swipeDist > 70) {
        // Deslizou para Direita: Se Colab estiver aberto, fecha. Se estiver fechado, abre o Menu Projetos.
        ...
        // Deslizou para Esquerda: Se o Menu Projetos estiver aberto, fecha. Senão, abre a Colab (se houver projeto aberto).
