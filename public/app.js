...
// Limpeza Nuclear: Varrer TODOS os projetos locais para garantir que o usuário
// não deixe rastros (presença, locks de tela ou chamada) em NENHUMA conversa ao sair.
async function limparMinhaPresencaGlobal() {
    ...
    try { await Promise.all(promessas); } catch(e) { console.log("Erro na varredura de presença:", e); }
}

// O novo login requer aguardar a varredura global
async function adicionarPresencaLocal() {
    if (!usuarioAtual || idProjetoAtivo === null || idConversaAtiva === null || !window.projetos[idProjetoAtivo].id) return;
    
    // Antes de entrar na nova sala, assegure-se de que sumiu das antigas
    await limparMinhaPresencaGlobal();
...
window.addEventListener('pagehide', limparMinhaPresencaGlobal);
window.addEventListener('beforeunload', limparMinhaPresencaGlobal);
...
window.confirmarLogout = async function() {
    if (confirm("Tem certeza que deseja sair da sua conta?")) {
        // Agora desligamos a conexão com o banco ANTES de invalidar o token de logout
        if (unsubscribeProjetos) unsubscribeProjetos();
        if (unsubscribeConvites) unsubscribeConvites();
        if (unsubscribeNotificacoes) unsubscribeNotificacoes();
        if (unsubscribeUsuarios) unsubscribeUsuarios();
...
window.selecionarConversa = async function(indexProj, indexConv) {
    if (idProjetoAtivo !== indexProj || idConversaAtiva !== indexConv) {
        await window.limparMinhaPresencaGlobal(); // Garante q a presença antiga morra
    }
